...
use crate::{buffer::SSBO, egui_tools::EguiRenderer, generate_smooth_gradient_field, shader::create_shader_module, simulation::{Simulation, SimulationSettings, TickSettings}, uniform::Uniform, Image};
...
impl Renderer {
    pub async fn new(window: Window, settings: SimulationSettings) -> Self {
        ...
        let simulation = Simulation::new(&device, settings);
    ...
    pub fn restart_simulation(&mut self) {
        self.simulation = Simulation::new(&self.device, self.sim_settings)
    }
